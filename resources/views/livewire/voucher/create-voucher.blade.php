<div class="w-full">
    <div class="mb-8 flex items-center gap-4">
        ...
        <div>
            <flux:heading level="1">{{ __('Create Voucher') }}</flux:heading>
            <flux:text class="mt-2 text-sm">{{ __('Create a new insurance voucher for automatic discount calculation') }}</flux:text>
        </div>
    </div>
    ...
    <div class="grid gap-6 md:grid-cols-3">
        ...
        <div class="md:col-span-2 space-y-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-950">
            <form wire:submit="create" class="space-y-6">
                ...
                <div>
                    <flux:checkbox
                        wire:model="isActive"
                        :label="__('Activate this voucher immediately')"
                    />
                    @error('isActive')
                        <flux:error>{{ $message }}</flux:error>
                    @enderror
        ...
        <div class="space-y-4">
            ...
            <div class="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-900 dark:bg-blue-950">
                <flux:heading level="4" class="mb-2 flex items-center gap-2">
                    <flux:icon icon="information-circle" class="h-5 w-5" />
                    {{ __('Tips') }}
                </flux:heading>
                <flux:text class="text-xs space-y-2">
                    <div>• {{ __('Set percentage discount for volume-based offers') }}</div>
                    <div>• {{ __('Use fixed amount for specific insurance partnerships') }}</div>
                    <div>• {{ __('Add max discount cap to prevent excessive deductions') }}</div>
                </flux:text>
            </div>
        </div>
    </div>
